"""
Banc d'essai OCR Multi-moteurs - Phase 5 (Isolation par sous-processus & Alignement exact)
Évalue chaque moteur dans un sous-processus indépendant pour éviter les conflits CUDA/PyBind.
Calcule le CER / WER avec alignement ligne par ligne pour une mesure exacte.
"""
...
def evaluate_ground_truth_matching(gt_lines: List[str], ocr_full_text: str) -> Tuple[float, float]:
    """
    Aligne chaque ligne du Ground Truth avec la meilleure ligne correspondante
    dans l'extraction OCR pour calculer le vrai CER et WER.
    """
...
# -------------------------------------------------------------
# Exécuteurs de sous-processus isolés
# -------------------------------------------------------------
...
"""
    ...
    res = subprocess.run([sys.executable, "-c", script], capture_output=True, encoding='utf-8', errors='replace', env=env)
...
def run_isolated_benchmark():
    print("=" * 90)
    print("PHASE 5 — BANC D'ESSAI OCR COMPARATIF (ISOLATION PROCESSUS & MESURE EXACTE)")
    print("=" * 90)
    ...
    for engine_name, backend, worker_fn in engines:
        ...
        for item in manifest:
            ...
            # Métriques réelles par alignement
    ...
    # Calcul des synthèses et scores
    summary = []
    for s in all_engine_results:
        ...
        # Formule : 35% Texte (1-WER) + 25% Nombres + 25% Caractères (1-CER) + 15% Vitesse
    ...
    out_json = Path("reports") / "phase5_final_benchmark_results.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump({"summary": summary, "details": all_engine_results}, f, indent=2, ensure_ascii=False)
        
    print("\n" + "=" * 105, flush=True)
    print("BILAN COMPARATIF FINAL DES MOTEURS OCR (PHASE 5)", flush=True)
    print("=" * 105, flush=True)
    print(f"{'Moteur':<20} | {'Backend':<12} | {'AR Précision':<12} | {'AR Nombres':<10} | {'AR Temps':<8} | {'FR Précision':<12} | {'FR Nombres':<10} | {'FR Temps':<8} | {'Score AR':<8} | {'Score FR':<8}", flush=True)
    print("-" * 130, flush=True)
    for st in summary:
        print(f"{st['engine']:<20} | {st['backend']:<12} | {st['ar_precision_wer']*100:6.1f}%     | {st['ar_num']*100:8.1f}% | {st['ar_time']:6.2f}s | {st['fr_precision_wer']*100:6.1f}%     | {st['fr_num']*100:8.1f}% | {st['fr_time']:6.2f}s | {st['ar_score']*100:6.1f}% | {st['fr_score']*100:6.1f}%", flush=True)
