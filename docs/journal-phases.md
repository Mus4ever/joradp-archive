> **Note (septembre 2026).** Ce document est le journal historique des phases
> du projet (preuves, mesures, décisions, corrections). Les scripts de
> validation mentionnés (`check_sequence_continuity.py`, `cross_validate_sample.py`,
> `verify_downloaded_pdfs.py`, `download_report.py`, `verify_sha256_redownload.py`,
> `coverage_report.py`, `verify_full_options.py`) ont été exécutés pendant les
> phases et ne font pas partie du dépôt final ; leurs résultats sont consignés
> ci-dessous. Le constat de structure du site, maintenu à jour, se trouve dans
> `SITE_STRUCTURE.md` à la racine.
>
> **Note complémentaire.** L'extraction de texte a été réalisée hors dépôt via
> Mistral OCR (FR 1962-2001 et tout l'AR) et extraction texte native Python
> (FR 2002-2026) ; les sorties sont dans `Extraction/` (non versionné).
